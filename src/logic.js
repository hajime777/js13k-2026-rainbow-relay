...
if(typeof document!='undefined')(()=>{const T={ja:['もっと先にいるよ！','虹をたどって僕を見つけて！','セクションクリア！','ゴール！','時間切れ！','次のステージ','開始','再開'],en:["I'm farther ahead!",'Follow the rainbow and find me!','SECTION CLEAR!','GOAL!','TIME UP!','Next Stage','Start','Restart']},K=Object.keys(T),M={'もっと先にいるよ！':0,"I'm farther ahead!":0,'虹をたどって僕を見つけて！':1,'Follow the rainbow and find me!':1,'SECTION CLEAR!':2,'セクションクリア！':2,'GOAL!':3,'ゴール！':3,'TIME UP!':4,'時間切れ！':4,'Next Stage':5,'次のステージ':5,'Start':6,'開始':6,'Restart':7,'再開':7},E=[document.querySelector('#clearText'),document.querySelector('#next'),document.querySelector('#reset')],t=document.querySelector('.title'),s=document.createElement('span');let L=(navigator.languages||[navigator.language]).map(x=>x.slice(0,2)).find(x=>T[x])||'en';function f(){document.documentElement.lang=L;s.textContent=' '+L.toUpperCase();for(const e of E){let v=e.textContent,m=v.match(/^(?:Stage|ステージ) (\d+) (?:CLEAR|クリア) - (\d+) (?:Sections|セクション)$/);if(m)v=L==='ja'?`ステージ ${m[1]} クリア - ${m[2]} セクション`:`Stage ${m[1]} CLEAR - ${m[2]} Sections`;else{let i=M[v];if(i!=null)v=T[L][i]}if(e.textContent!==v)e.textContent=v}}new MutationObserver(f).observe(document.body,{childList:true,characterData:true,subtree:true});s.style.cursor='pointer';s.onclick=()=>{L=K[(K.indexOf(L)+1)%K.length];f()};t.append(s);f()})()
